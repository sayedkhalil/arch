...
const[activ,setactiv]=useState(Data.a)
...
<ul className={`nav justify-content-center  ${styles.navdesk}`}>
  {/* {activ.map(x=>(
     <li className="nav-item " key={x.path}>
     <Link href={`/Project/${x.path}`}>
    ...
    <select class="form-select w-100 p-1 " aria-label="Default select example" onChange={routering}>
  <option selected>مشروع</option>
  {activ.map(x=>  <option  value={x.path}>{x.project}</option>
) }
...
<li className="nav-item ">
    <Link href="/pmo">
    <a className="nav-link ul-text " >المعالم</a>
    </Link>   
  </li>
 <li className="nav-item ">
    <Link href="/">
   ...
   {/* <Link href='/cart'>
    ...
    <div className="  navbar-collapse " >
      <ul className={`navbar-nav ${nav} `}>
        ...
        {activ.map(x=>(
     <li className="nav-item ms-auto " key={x.path}>
     <Link href={`/Project/${x.path}`}>
     <a className="nav-link active" aria-current="page" >{x.project}</a>
     </Link>  
   </li>
  ))}
